...
uint128 mips_multiplication(T x, T y) {
    return static_cast<doubled_t<T>>(x) * static_cast<doubled_t<T>>(y);
}
...
uint128 mips_division(T x, T y) {
    ...
    if (y == 0)
        return 0;
    auto x1 = static_cast<T2>(x);
    auto y1 = static_cast<T2>(y);
    return (static_cast<uint128>(static_cast<T2>(x1 / y1)) & bitmask<uint128>(bitwidth<T>))
         | (static_cast<uint128>(static_cast<T2>(x1 % y1)) << bitwidth<T>);
}
...
class MIPSInstr
{
    private:
        ...
        template <typename T>
        void execute_sub()   { v_dst = static_cast<T>( static_cast<sign_t<T>>( v_src1) - static_cast<sign_t<T>>( v_src2)); }
        ...
        template <typename T>
        void execute_addiu() { v_dst = static_cast<T>(static_cast<sign_t<T>>(v_src1) + static_cast<sign_t<T>>( sign_extend(v_imm))); }
