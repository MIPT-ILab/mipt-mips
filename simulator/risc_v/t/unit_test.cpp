...
TEST_CASE( "RISV RV32 packh")
{
    CHECK( RISCVInstr<uint32>(0x04d63d33).get_disasm() == "packh $a0, $a2, $a3");
    RISCVInstr<uint32> instr( "packh", 0);
    instr.set_v_src( 0xffff'2222, 0);
    instr.set_v_src( 0x1111'3333, 1);
    instr.execute();
    CHECK( instr.get_v_dst( 0) == 0x0000'0032);
}

TEST_CASE( "RISV RV64 packh")
{
    RISCVInstr<uint64> instr( "packh", 0);
    instr.set_v_src( 0xffff'ffff'2222'2222, 0);
    instr.set_v_src( 0x1111'1111'3333'3333, 1);
    instr.execute();
    CHECK( instr.get_v_dst( 0) == 0x0000'0000'0000'0032);
}
...
TEST_CASE("RISCV RV32 gorci")
{
    ...
    std::vector<uint32> shamts{0x1, 0x2, 0x5, 0xC, 0xFF, 0x3};
    for(std::size_t i = 0; i < cases.size(); i++) {
        INFO( "Iteration: " << i);
