    ...
    constexpr bool check_mask( uint32 bytes, uint16 base_supported, uint16 base) const noexcept
    {
        if ( ( bytes & mask) == match)
            switch(base_supported)
            {
                ...
                case RV32:       return base == 32;
                case RV64:       return base == 64;
                case RV128:      return base == 128;
                case RV32_RV64:  return ( base == 32) || ( base == 64);
                case RV64_RV128: return ( base == 64) || ( base == 128);
            }
        return false;
    }
};
