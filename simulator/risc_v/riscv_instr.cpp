...
struct RISCVAutogeneratedTableEntry
{
    ...
    constexpr bool check_mask( uint32 bytes, uint16 base_supported, uint16 base) const noexcept
    {
        if ( ( bytes & mask) == match)
            switch(base_supported)
            {
                case RV_ANY:     return 1;
                case RV32:       if ( base == 32) return 1; break;
                case RV64:       if ( base == 64) return 1; break;
                case RV128:      if ( base == 128) return 1; break;
                case RV32_RV64:  if ( ( base == 32) || ( base == 64)) return 1; break;
                case RV64_RV128: if ( ( base == 64) || ( base == 128)) return 1; break;
            }
        return 0;
    }
};
...
const auto& find_entry( uint32 bytes, uint16 base)
{
    for (const auto& e : cmd_desc<I>)
        if ( e.entry.check_mask( bytes, e.base, base))
            return e;
