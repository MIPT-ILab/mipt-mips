...
{
    auto test_lru_module = create_cache_replacement( "LRU", 3);
    auto test_pseudo_lru_module = create_cache_replacement( "Pseudo-LRU", 3);
    CHECK(test_lru_module->get_ways() == 3);
    CHECK(test_pseudo_lru_module->get_ways() == 3);
}
...
{
    auto test_pseudo_lru_module = create_cache_replacement( "Pseudo-LRU", 3);
    CHECK_THROWS_AS( test_pseudo_lru_module->allocate( 3), CacheReplacementException);
    CHECK_THROWS_AS( test_pseudo_lru_module->set_to_erase( 3), CacheReplacementException);
}

TEST_CASE( "Pseudo-LRU: Check_bad_way_number_in_touch_method")
{
    auto test_pseudo_lru_module = create_cache_replacement( "Pseudo-LRU", 3);
    CHECK_THROWS_AS( test_pseudo_lru_module->touch( 4), CacheReplacementException);
}
...
TEST_CASE( "Pseudo-LRU: Check_touch_method_with_odd_number_of elements")
{
    auto test_lru_module = create_cache_replacement( "Pseudo-LRU", 3);
    test_lru_module->touch( 2);
    test_lru_module->touch( 1);
