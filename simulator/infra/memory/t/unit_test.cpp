...
TEST( Func_memory, Read_Method_Test)
{
    ...
    ASSERT_EQ( func_mem.read<uint32>( dataSectAddr), right_ret);

    // read 3 bytes from the func_mem start addr + 1
    right_ret = 0x030201;
    ASSERT_EQ( func_mem.read<uint32>( dataSectAddr + 1, 0xFFFFFFull), right_ret);

    // read 2 bytes from the func_mem start addr + 2
    right_ret = 0x0302;
    ASSERT_EQ( func_mem.read<uint32>( dataSectAddr + 2, 0xFFFFull), right_ret);
    ASSERT_EQ( func_mem.read<uint16>( dataSectAddr + 2), right_ret);

    // read 1 bytes from the func_mem start addr + 3
    right_ret = 0x03;
    ASSERT_EQ( func_mem.read<uint8>( dataSectAddr + 3), right_ret);

    // check hadling the situation when read
    // from not initialized or written data
    ASSERT_EQ( func_mem.read<uint8>( 0x300000), NO_VAL8);
}
...
TEST( Func_memory, Write_Read_Initialized_Mem_Test)
{
    ...
    ASSERT_EQ( func_mem.read<uint32>( data_sect_addr), right_ret);
...
TEST( Func_memory, Write_Read_Not_Initialized_Mem_Test)
{
    ...
    ASSERT_EQ( func_mem.read<uint16>( write_addr), right_ret);

    right_ret = 0x0201;
    ASSERT_EQ( func_mem.read<uint16>( write_addr + 1), right_ret);
