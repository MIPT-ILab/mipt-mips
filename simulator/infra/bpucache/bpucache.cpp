...
uint32 BPUCache::check_arguments( uint32 size, uint32 ways)
{
    if ( size == 0)
...
std::pair<Addr, uint32> BPUCache::read( Addr addr)
{
...
std::pair<Addr, uint32> BPUCache::read_no_touch( Addr addr) const
{
    uint32 num_set = set( addr);
...
uint32 BPUCache::write( Addr addr)
{
    uint32 num_set = set( addr);
