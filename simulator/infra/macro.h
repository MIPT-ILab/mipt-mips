...
public:
    ...
    explicit StringView( std::nullptr_t) : value( nullptr) { }
    explicit StringView( const char* v) : value( v) { }

    // The pointer is guaranteed to point to static string
    // so it is OK to copy it by value
    StringView& operator=( const StringView& v) = default;
    StringView& operator=( const char* v) {
        value = v;
        return *this;
    }
