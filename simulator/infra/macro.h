...
uint64_t shuffle_stage( uint64_t src, uint64_t maskL, uint64_t maskR, size_t N)
{
...
std::array<uint64_t> shfl_consts[] = {
...
auto bit_shuffle( T src1, T src2)
{
    auto pre_result = src1;
    auto shamt = src2 & ( ( bytewidth<T> > bytewidth<uint32_t>) ? 31 : 15);
    size_t offset = bytewidth<T> / bytewidth<uint64_t>;
    size_t limit = 4 + offset;
    for( size_t i = 0; i < limit ; ++i)
    {
        if ( shamt & ( 1 << ( limit - i - 1)))
            pre_result = shuffle_stage( pre_result,
                                            shfl_consts[2 * i + offset * 8],
                                            shfl_consts[2 * i + offset * 8 + 1], 
                                            1 << ( limit - i - 1));
    }
    return pre_result;
}

uint128 bit_shuffle( uint128 src1, uint128 src2)
{
    uint128 result = src1;
    uint128 shamt = src2 & 63;
    uint64_t left_part = result >> 64;
    uint64_t right_part = result;
    for( size_t i = 0; i < 5; ++i)
    {
        if( shamt & ( 1 << ( 4 - i)))
        {
            left_part = shuffle_stage( left_part,
                                       shfl_consts[2 * i + 8],
                                       shfl_consts[2 * i + 8],
                                       1 << (4 - i));
            right_part = shuffle_stage( right_part,
                                        shfl_consts[2 * i + 8],
                                        shfl_consts[2 * i + 8],
                                        1 << (4 - i));
        }
    ...
    if( shamt & (1 << 5))
    {
        uint64_t buf = left_part & bitmask<uint64_t>(32);
	left_part = (left_part & ~(bitmask<uint64_t>(32)) | (right_part & (bitmask<uint64_t>(32) << 32));
        right_part |= buf << 32;
    }
    result = left_part;
    result <<= 64;
